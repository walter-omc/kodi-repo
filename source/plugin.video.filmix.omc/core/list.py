    ...
    def add_movies(self, response, ifempty=30111):
        if(len(response['data']) > 0):
            for movie in response['data']:
				...
				$LOCALIZE[515]
				"""
                movie['plot'] = "[COLOR button_focus]" + xbmcup.app.lang[30138] + ": [/COLOR]" + movie['genre'] + "[CR][CR]" + movie['plot']
				# + ' ' + movie['year'] + ' ' + movie['quality']
                rating_str = ''
                if not movie['rating'] == 0:
				    rating_str = '(' + 'R ' + '[COLOR ' + ('green' if movie['rating'] > 0 else 'red') + ']' + str(movie['rating']) + '[/COLOR]' + ')'

                self.item(not_movie + movie['title'] + ' ' + movie['quality'] + '  ' + rating_str,
...
class MovieList(AbstactList):
    def handle(self):
        ...
        md5 = hashlib.md5()        
        nocache = "&tme=" + str(time.time())
        nocache = ""
        md5.update(page_url+'/page/'+str(page)+'?v='+xbmcup.app.addon['version'] + nocache)
...
class SearchList(AbstactList):
    def handle(self):
        ...
        search_params['story'] = urllib.unquote(params['usersearch'])

        '''
            Remove fake unicode flag
        '''
        
        try:
            vbad = repr(search_params['story'])
            if vbad[0:2] == 'u\'':
                v = vbad[2:-1]
                final = ""
                accum = ""
                for s in v.split('\\x'):
                    ...
                    accum += chr(int(s, 16))
                    if len(accum) == 2:
                        final += accum.decode('utf-8')
                        accum = ""
                
            search_params['story'] = final.encode('utf-8')
            
            #search_params['story'] = search_params['story'].encode('utf-8')
            #search_params['story'] = search_params['story'].decode('utf-8')
        except:
            pass
        
        # search_params['search_start'] = 0 if (page == 0 or page == 1) else page + 1
        search_params['search_start'] = page + 1
        
        # self.dev_log({ 'search_params': search_params })

        url_post_params = '-'.join(search_params)
        
        # page_url = "/engine/ajax/sphinx_search.php?scf=fx&story=%s&search_start=%s" % (params['usersearch'], page)
        # page_url = "/search/%s" % params['usersearch']
        
        # page_url = "/search/%s" % search_params['story']
        
        page_url = "/engine/ajax/sphinx_search.php"
        #page_url = "/engine/ajax/sphinx_search.php"
        ...
        nocache = "&tme=" + str(time.time())
        nocache = ""
        md5.update(params['usersearch'].encode('utf8') + '?v='+xbmcup.app.addon['version'] + '&p=' + '-'.join(search_params) + '&page=' + str(page + 1) + nocache)
        response = CACHE(str(md5.hexdigest()), self.get_movies_simple_search, page_url, search_params)
        ...
        if(is_united_search == 0):
            if(response['page']['maxpage'] >= response['page']['pagenum']+1):
                self.item(u'[COLOR green]'+xbmcup.app.lang[30107] + ' ' + str(response['page']['pagenum']+1) + '/' + str(response['page']['maxpage']) + '[/COLOR]', self.replace('search', params), cover=cover.next, folder=True)
...
class QualityList(xbmcup.app.Handler, HttpData, Render):
    ...
    def add_playable_item(self, movie):
        ...
        # self.dev_log({ 'add_playable_item ': movie })
